...
void loop()
{
    ...
    {                                                  // Update computer every 0.1 seconds
...
void checkSerial()
{
    ...
    { // If data is available on serial port
        ...
        {
        case 0x00:
            pendulum.setMagnitude(value);
            break;
        case 0x01:
            pendulum.setSpeed(tmc, value);
            break;
        case 0x02:
            pendulum.setAcceleration(tmc, value);
            break;
        case 0x03:
            pendulum.setThreshold(value);
            break;
        case 0x06:
            pendulum.setBalancePos(value);
            break;
        case 0x07:
            pendulum.setLimit(value);
            break;
        case 0x08:
            pendulum.setLoopImpulse1(value);
            break;
        case 0x09:
            pendulum.setLoopImpulse2(value);
            break;
        case 0x0A:
            pendulum.setLoopAngle(value);
            break;
        case 0x20:
            pendulum.setKpa(value);
            break;
        case 0x21:
            pendulum.setKda(value);
            break;
        case 0x22:
            pendulum.setKpm(value);
            break;
        case 0x23:
            pendulum.setKdm(value);
            break;
        case 0x50:
            setMoveMode(static_cast<MoveMode>(data[1]));
            break;
        case 0x51:
            resetTMC();
            break;
...
void oscillation()
{
    // Protocol mode 1 now performs closed-loop energy shaping.
    if (pendulum.overLimit())
    {
        setMoveMode(STANDBY);
        return;
    }
    if (pendulum.readyForUpright())
    {
        setMoveMode(INVERTEDBALANCE);
        return;
    }
    pendulum.swingUp(tmc);
}
void invertedBalance()
{
    if (pendulum.overLimit())
    ...
    // Hysteresis prevents rapid switching around the capture boundary.
    if (pendulum.uprightLost())
    {
        setMoveMode(OSCILLATION);
        return;
    }
...
void looping(uint8_t direction = CW)
{
    ...
    case 1:
        ...
        if (pendulum.loopingTest2())
        {
            phaseCounter = 2;
        }
        break;
    case 2:
        if (pendulum.loopingTest3())
        {
            setMoveMode(INVERTEDBALANCE);
        }
        break;
...
void resetTMC()
{
    ...
    // Scaler 192/256 as in the tests; init() applies it before enabling the
    // outputs, so it is not a separate call afterwards.
