...
# ==========================================
params = {
    # 물리 모델: 스텝모터가 직접 가속도를 만든다고 보고 g, L 중심으로 단순화
    "L": 0.3, "g": 9.81,
    "x0": 0.0,
...
def compute_solution(p):
    ...
    def nonlinear_dynamics(t, X, u):
        ...
        # 가속도 제어 스텝모터: 입력 u를 카트의 직접 가속도로 취급
        u = float(np.clip(u, -p["u_max"], p["u_max"]))

        # 가상 벽
        wall_acc = 0.0
        if x < p["rail_x_min"]:
            wall_acc = 5000.0 * (p["rail_x_min"] - x) - 100.0 * x_dot
            if u < 0: u = 0.0
        elif x > p["rail_x_max"]:
            wall_acc = 5000.0 * (p["rail_x_max"] - x) - 100.0 * x_dot
            if u > 0: u = 0.0

        disturbance = p["dist_amp"] * np.sin(2.0 * np.pi * p["dist_freq"] * t)
        u_eff = u + disturbance + wall_acc

        x_ddot = u_eff - p["cart_damping"] * x_dot
        theta_ddot = (3.0 * g / (2.0 * L)) * np.sin(theta) - (3.0 / (2.0 * L)) * u_eff * np.cos(theta) - p["joint_damping"] * theta_dot
    ...
    def closed_loop(t, X):
        if t + 1e-12 >= controller["next_t"]:
            ...
            if abs(q_theta_wrap) < p["catch_angle"]:
                ...
                energy = 0.5 * (p["L"]**2) * est_theta_dot**2 + p["g"] * p["L"] * (np.cos(q_theta_wrap) - 1.0)
