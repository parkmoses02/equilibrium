...
*
* 이 브랜치는 SD_MODE = 0 보드(TMC5160_BOB_V1.0 등) 전용이다. 칩 내장 모션
* 컨트롤러(RAMPMODE / XTARGET / VMAX / AMAX)가 가감속과 위치 카운트를 모두
* 처리한다. SD_MODE 가 기판에서 HIGH 로 묶인 보드(BIGTREETECH TMC5160T Pro 등)
* 에서는 이 방식으로 모터가 돌지 않는다 - 그 경우는 stepdir 브랜치를 쓸 것.
*/
#include <Arduino.h>
...
class TMC {
    ...
    void init(float iHold_=0.05, float iRun_=0.6, float mStep_=0, uint8_t scaler_=192);
