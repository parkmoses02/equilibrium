...
- 구동 백엔드: `lib/TMC/TMCStepDir.{h,cpp}` (베이스 클래스 `lib/TMC/TMC.{h,cpp}`)
- `src/main.cpp` 는 `TMCStepDir` 로 고정되어 있다. magdi 브랜치의
  `USE_STEPDIR_BACKEND` 분기는 여기서 제거했다.
...
| `tmc5160_diagnostics` | SPI 레지스터 진단 (SD_MODE 판정 포함) |
